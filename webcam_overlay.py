...
def capture_frames():
    ...
    global latest_frame, is_recording, recording_writer, recording_filename, recording_frame_count
    ...
    print(f"Webcam opened: {STREAM_WIDTH}x{STREAM_HEIGHT} @ {STREAM_FPS}fps")
...
''';
        ...
        <a href="/webcam-viewer/" class="back-link">← Back to Webcam Stream</a>
            ...
            try:
                ...
                if 'reset' in params:
                    # Reset to defaults
                    subprocess.run(['v4l2-ctl', f'--device={device}', '--set-ctrl=zoom_absolute=100'], 
                                 capture_output=True)
                    subprocess.run(['v4l2-ctl', f'--device={device}', '--set-ctrl=pan_absolute=0'], 
                                 capture_output=True)
                    subprocess.run(['v4l2-ctl', f'--device={device}', '--set-ctrl=tilt_absolute=0'], 
                                 capture_output=True)
                    subprocess.run(['v4l2-ctl', f'--device={device}', '--set-ctrl=brightness=128'], 
                                 capture_output=True)
                    subprocess.run(['v4l2-ctl', f'--device={device}', '--set-ctrl=contrast=128'], 
                                 capture_output=True)
                    result['reset'] = True
